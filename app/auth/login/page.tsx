...
function LoginContent() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Hardcode the site URL to ensure correct redirection
      const siteUrl = 'https://iphone-url-sharing-consolidate-johngleason-outlookcoms-projects.vercel.app';
      
      console.log(`Redirecting to: ${siteUrl}/auth/callback?returnUrl=${returnUrl}`);
      
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${siteUrl}/auth/callback?returnUrl=${returnUrl}`,
        },
      });
